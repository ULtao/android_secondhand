...
public class CommodityDbHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(CREATE_COMMODITY_DB);//创建数据表，参数为要执行的sql语句
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        Log.i(" 商品表",oldVersion + "-->" +newVersion);
    }
     ...
     * @return 商品列表
     */
    public List<Commodity> readCommodityTitle(String title1) {
        List<Commodity> differentTitle = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery("select * from tb_commodity where title=?",new String[]{title1});
        if(cursor.moveToFirst()) {
            do{
                String title = cursor.getString(cursor.getColumnIndex("title"));
                float price = cursor.getFloat(cursor.getColumnIndex("price"));
